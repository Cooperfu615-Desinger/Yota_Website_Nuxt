...
<template>
  <div class="lobby-page bank-page px-4 py-5">
    <template v-if="!isLoggedIn"><div class="card-purple p-8 text-center max-w-sm mx-auto mt-8"><div class="text-5xl mb-4">🏦</div><h1 class="text-xl font-black mb-2">銀行</h1><p class="text-sm mb-5" style="color:var(--color-text-muted);">登入後即可使用官網儲值渠道並查看專屬優惠</p><button class="btn-gold w-full justify-center" @click="openLogin(route.fullPath)">立即登入 / 註冊</button></div></template>
    <template v-else>
      <header class="bank-hero"><div><p>JH BANK</p><h1>銀行</h1><span>保留官網專屬儲值渠道與優惠內容</span></div><WalletBalances :user="userInfo" variant="cards" /></header>
