...
const vipUpgrade = computed(() => userInfo.value.vip === 0
  ? { ...vipUpgradeSeed, historicalDeposit: 0, historicalWager: 0, monthlyDeposit: 0, monthlyWager: 0, activeDays: 0, highestVip: 0, upgradeProtection: false, deposit: { current: 0, target: nextVipLevel.value?.historicalDeposit ?? 0 }, wagerGold: { current: 0, target: vipUpgradeSeed.wagerGold.target }, wagerSilver: { current: 0, target: vipUpgradeSeed.wagerSilver.target } }
  : vipUpgradeSeed)
...
const goldWagerPct = computed(() => vipUpgrade.value.wagerGold.target ? Math.min(100, Math.round(vipUpgrade.value.wagerGold.current / vipUpgrade.value.wagerGold.target * 100)) : 0)
const silverWagerPct = computed(() => vipUpgrade.value.wagerSilver.target ? Math.min(100, Math.round(vipUpgrade.value.wagerSilver.current / vipUpgrade.value.wagerSilver.target * 100)) : 0)
        ...
        <div class="member-profile-panel">
          ...
          <section v-else-if="activeSection === 'vip'" class="member-content vip-content">
            ...
            <div class="vip-main-grid">
              ...
              <article class="vip-panel vip-upgrade-panel">
                <div v-if="isMaxVip" class="vip-max-state">已達目前最高等級</div>
                <div v-else-if="isVip2Undefined" class="vip-undefined-state">VIP2 升級條件待設定</div>
                <template v-else>
                  <div class="vip-next-line"><span>下一級 VIP {{ nextVipLevel.level }}・{{ nextVipLevel.name }}</span><strong>{{ nextVipLevel.upgradeRequirement }}</strong></div>
                  <div class="vip-progress-row"><span>歷史儲值</span><b>{{ vipUpgrade.deposit.current.toLocaleString() }} / {{ nextVipLevel.historicalDeposit?.toLocaleString() }}</b><i><em :style="{ width: `${depositPct}%` }" /></i></div>
                  <div class="vip-progress-row"><span>當月有效投注・金幣</span><b>{{ vipUpgrade.wagerGold.current.toLocaleString() }} / {{ vipUpgrade.wagerGold.target.toLocaleString() }}</b><i><em :style="{ width: `${goldWagerPct}%` }" /></i></div>
                  <div class="vip-progress-row"><span>當月有效投注・銀幣</span><b>{{ vipUpgrade.wagerSilver.current.toLocaleString() }} / {{ vipUpgrade.wagerSilver.target.toLocaleString() }}</b><i><em :style="{ width: `${silverWagerPct}%` }" /></i></div>
                  <small class="vip-muted">資料要求：無額外要求</small>
                </template>
              </article>
              <article class="vip-panel vip-maintain-panel"><div class="vip-maintain-status" :class="{ ready: maintainReady }">{{ vipUpgrade.upgradeProtection ? '本月升級保護中' : maintainReady ? '本月已達保級條件' : '本月保級進度' }}</div><div class="vip-progress-row"><span>月儲值</span><b>{{ vipUpgrade.monthlyDeposit.toLocaleString() }} / {{ currentVipLevel.monthlyDeposit?.toLocaleString() || '無條件' }}</b><i><em :style="{ width: `${maintainDepositPct}%` }" /></i></div><div class="vip-progress-row"><span>月投注</span><b>{{ vipUpgrade.monthlyWager.toLocaleString() }} / {{ currentVipLevel.monthlyWager?.toLocaleString() || '無條件' }}</b><i><em :style="{ width: `${maintainWagerPct}%` }" /></i></div><div class="vip-progress-row"><span>活躍天數</span><b>{{ vipUpgrade.activeDays }} / {{ currentVipLevel.activeDays || '無條件' }} 天</b><i><em :style="{ width: `${maintainActivePct}%` }" /></i></div></article>
            </div>
          </section>
...
.vip-next-line span { font-size:15px; }
.vip-maintain-status { font-size:15px; }
