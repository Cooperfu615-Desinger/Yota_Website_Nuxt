  ...
  <div v-else class="member-profile-view" :class="{ 'member-profile-view--embedded': props.embedded }">
    ...
    <section v-else-if="activeSection === 'vip'" class="member-content vip-content">
      ...
      <div class="vip-main-grid">
        ...
        <article class="vip-panel vip-benefit-panel"><div class="vip-panel-label">等級權益</div><div class="vip-benefit-row"><span>P2P 贈禮手續費</span><strong>{{ currentVipLevel.p2pFee }}</strong></div><div class="vip-benefit-row"><span>本級升級獎勵</span><strong>{{ currentVipLevel.upgradeReward }}</strong></div><div class="vip-benefit-row"><span>獎勵狀態</span><strong>{{ userInfo.vip === 0 ? '不適用' : '已發放' }}</strong></div><small class="vip-muted">已取得的同級獎勵不重複發放</small></article>
